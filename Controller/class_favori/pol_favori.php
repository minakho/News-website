...
class pol_favori {
    var $feed;
    function pol_favori($feed) {
        $this->feed = $feed;
    }
    function parse() {
        $pol_favori = simplexml_load_file($this->feed);
        $pol_favori_split = array();
        foreach ($pol_favori->channel->item as $item) {
        ...
        return $pol_favori_split;
    }
    function display($numrows, $head) {
        $pol_favori_split = $this->parse();
        $i = 0;
        $pol_favori_data = '
    <div class="vas">
        ...
        while ($i < $numrows) {
            $pol_favori_data .= $pol_favori_split[$i];
            $i++;
        }
        $trim = str_replace('', '', $this->feed);
        $user = str_replace('&lang=en-us&format=pol_favori_200', '', $trim);
        $pol_favori_data.='</div></div>';
        return $pol_favori_data;
    }
}
?>
