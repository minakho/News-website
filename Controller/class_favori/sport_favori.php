...
class sport_favori {
    var $feed;
    function sport_favori($feed) {
        $this->feed = $feed;
    }
    function parse() {
        $sport_favori = simplexml_load_file($this->feed);
        $sport_favori_split = array();
        foreach ($sport_favori->channel->item as $item) {
        ...
        return $sport_favori_split;
    }
    function display($numrows, $head) {
        $sport_favori_split = $this->parse();
        $i = 0;
        $sport_favori_data = '
    <div class="vas">
        ...
        while ($i < $numrows) {
            $sport_favori_data .= $sport_favori_split[$i];
            $i++;
        }
        $trim = str_replace('', '', $this->feed);
        $user = str_replace('&lang=en-us&format=sport_favori_200', '', $trim);
        $sport_favori_data.='</div></div>';
        return $sport_favori_data;
    }
}
?>
